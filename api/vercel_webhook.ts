...
async function purgeCloudflareCache() {
  ...
  const webOrigin = process.env.WEB_ORIGIN ?? "http://127.0.0.1"
  const manifestPath = await fetch(`${webOrigin}/assets/manifest.txt?t=${Date.now()}`).then((res) =>
    res.text(),
  )
