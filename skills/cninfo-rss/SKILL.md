...
- 命中**分类码**后还要过**分类内标题二次校验**（`title_include_any` /
  `title_exclude_any`）才算准入——分类码只是「粗筛」，标题规则才是「准入门」。
- 命中分类（且过准入门）或 `include_any` 标题词 → 标候选：
  `evidence_layer=L1_L3_candidate` + `fact_hardness=review_candidate` +
  `review_required=true`（待 review）。
- 标题含低确定性词（拟/计划/预计/有望/框架协议…）→ 降级
  `update_type=review_candidate` + `confidence=low`，**不进 entity 正文**。
- **组合规则收紧 hard_delta**：「宽词」（签订/产能/亏损/增持/减持）单独命中只算
  `review_candidate`；标题须含 `hard_delta_combo_rules` 同组伴随词（如 签订+合同/订单、
  产能+投产/达产）才升级 `hard_delta` + `confidence=high`。
...
| `l3_categories`                                 | 巨潮分类码 + fact_types + enabled + 分类内标题准入门   |
| `l3_categories[].title_include_any/exclude_any` | 分类命中后的标题二次校验（粗筛 → 准入门）              |
...
| `hard_delta_combo_rules`                        | 宽词需配伴随词才升 hard_delta，否则降 review_candidate |

## 输出

- `feeds/l3-candidates-hard-delta.xml` — 全市场 L3 候选 · 高确定性合集（`update_type=hard_delta` 的命中；旧名 `l3-hard-delta.xml`，重命名避免误读为已验证硬事实）
- `feeds/by-category/{code}.xml` — 每个订阅分类一份（hard + review）
- `feeds/by-fact-type/{fact_type}.xml` — 每个事实类型一份（细颗粒度订阅：订单合同/客户定点/注册获证/量产投产/增减持…各自独立 feed，hard + review 都进）
...
- `http://localhost:8787/l3-candidates-hard-delta.xml`（全市场 L3 候选 · 高确定性）
- `http://localhost:8787/by-category/category_gqjl_szsh.xml`（按分类，如股权激励）
- `http://localhost:8787/by-fact-type/order_contract.xml`（按事实类型，如重大合同/订单；另有 customer_validation / regulatory_approval / mass_production 等）
...
- 条件（When）：`feed_url` `contains` `l3-candidates-hard-delta`（或 `:8787`，按你的 feed 地址）
...
- **2026-06-30（准入收紧）**：dry-run 实测分类码命中即 hard_delta 太宽（近 3 天每源 1 页
  抓 201 条、hard_delta 132，混入担保进展/H 股招股/问询回复/董事会决议/风险提示等）。修正：
  - `category_yjkb_szsh`（业绩快报）实测返回大量全量公告 → 关闭，业绩快报改走标题关键词；
  - `category_zj_szsh` 巨潮官方含义是**中介报告**（非增减持）→ 改用 `category_gqbd_szsh`
    （股权变动）并要求标题含 增持/减持/权益变动/持股变动/股份变动；
  - 分类命中后加**标题二次校验**（`title_include_any/exclude_any`）；
  - 扩充 `exclude_any`（担保/问询函/关注函/回复/股东会决议/风险提示/专项说明/承诺函…）；
  - 新增 `hard_delta_combo_rules`：宽词（签订/产能/亏损/增持/减持）需配伴随词才升 hard_delta；
  - feed 重命名 `l3-hard-delta.xml → l3-candidates-hard-delta.xml`（避免误读为已验证硬事实）；
    **FinHot 订阅 URL 需同步改**为 `…/l3-candidates-hard-delta.xml`。
