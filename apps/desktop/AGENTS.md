...
Do not add MingCute Pro assets to the repository. Do not create `icons/mgc`, `i-mgc-*`, or `*-cute-*` icon names.

**Important**: Use the local Focal icon set (`i-focal-*`) for product UI icons. New generic icons should be backed by Lucide. Lucide state icons use stroke color plus background/container state, not filled icon variants. AI/LLM provider icons should come from `@lobehub/icons-static-svg`; other third-party brand marks should come from `simple-icons` or `logos`.

Examples:

- Product UI: `i-focal-copy`, `i-focal-external-link`, `i-focal-rada`
- Brand icons: `i-focal-github`, `i-focal-gemini`, `i-focal-deepseek`
...
- **Follow All Style Rules**: Must adhere to UIKit colors, approved open-source icon sources, and Framer Motion (`m.` prefix) guidelines
...
Focal uses a sophisticated glassmorphic depth design system for elevated UI components (modals, toasts, floating panels, etc.). This design provides visual hierarchy through layered transparency and subtle color accents.
